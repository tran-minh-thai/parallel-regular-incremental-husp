...
δ·U(ΔD), so each part of the database is mined at its own natural threshold, which is the condition the
partition lemma needs for the union of the two to be complete. Scenario S9 sweeps λ and shows recall
unchanged at 1.0 throughout, since correctness does not depend on it, while cost traces a U: seeding gets
...
LEVIATHAN. The point is not that 1 is optimal, but that 1 is *fixed by the lemma* rather than
