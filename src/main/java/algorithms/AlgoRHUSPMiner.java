...
/**
 * Unified RHUSP (Regular High Utility Sequential Patterns) mining engine
 ...
 * returns exactly the same RHUSP set (differing only in speed):
 *   - RHusp's original candidate condition:  utility(P) + remUtility(P) >= minUtility.
 *   - Regularity check when CONCLUDING a pattern: maxPeriod(P) <= maxRegularity.
 *
 * The RHusp baseline (Ishita et al., Appl Intell 2021) = {useEUCS=false, boundMode=PEU,
 * useRegPruning=true}. Measured time includes preprocessing (Tier 0 + EUCS construction);
 * the preprocessing portion is recorded separately in preprocessMs for separate reporting.
 *
 * Data structures:
 *   - CSR flat-array database: items/utils/remUtils/posEvent + sequence/event offsets.
 *   - Projected database: 3 parallel primitive arrays (sid, last item position, prefix utility).
 *   - EUCS: open-addressing primitive hash map keyed by pair (memory ~ number of pairs actually
 *     co-occurring, instead of a dense O(n^2) matrix that easily exhausts RAM), then converted to
 *     an adjacency list sorted descending by utility for early pruning via break.
...
public class AlgoRHUSPMiner {
    ...
    public long cpuStartNanos, cpuEndNanos;        // CPU time of the mining thread (robust to machine-sleep noise)
    ...
    // Background memory sampler: records peak heap regularly throughout the mining phase,
    // far more stable than sampling only at node boundaries.
    ...
    // ----- CSR database -----
    private int numSeq;
    private int[] seqPosStart;   // numSeq+1: offset of the first position of each sequence
    private int[] items;         // item id at each position (global)
    private long[] utils;        // utility at each position
    private long[] remUtils;     // remaining utility IMMEDIATELY AFTER the position (within the same sequence)
    private int[] posEvent;      // local event index of each position
    private int[] seqEvtStart;   // numSeq+1: offset of the first event of each sequence
    ...
    // Optional ABSOLUTE thresholds (for scalability testing: fix the same threshold
    // across all subsets so the pattern count is monotone in |D|). >= 0 means use it;
    // < 0 (default) means compute by ratio as usual.
    public long forcedMinUtil = -1;
    public int forcedMaxReg = -1;

    // ----- Tier 0 / EUCS -----
    private int[] itemToDense;   // -1 if the item is not in the potential set
    private int[] denseToItem;
    private int numValid;
    private int[] adjIStart, adjSStart;     // numValid+1
    private int[] adjIItem, adjSItem;       // original item id of the neighbor
    private long[] adjIUtil, adjSUtil;      // EUCS value, sorted descending within each slice

    // ----- Reusable buffers (avoid allocation in the recursion loop) -----
    private long[] seqBoundI, seqBoundS;    // per-sequence bounds
    private int[] seqVisitI, seqVisitS;     // stamp = sid; reset to -1 on flush
    private int[] seqActI, seqActS;
    private int seqNumI, seqNumS;
    private long[] gAccI, gAccS;            // global bound accumulation (sum over sequences)
    private int[] gMarkI, gMarkS;           // -1 = not yet present
    private int[] gActI, gActS;
    private int gNumI, gNumS;
    private long[] evtMaxUtil;              // largest candidate utility per event
    private int[] evtMaxPos;
    private int[] candBufI, candBufS;
    private int[] laStampArr;               // mark items valid for LA-PEU
    ...
    public void runAlgorithm(String dataPath, String utilPath, String outputFolder,
        ...
        resetState();
        loadData(dataPath, utilPath, minRatio, maxRegRatio); // data-read I/O excluded from runtime (uniform across configurations)
        allocateBuffers();
        MemMeter.reset(); // GC + reset the peak-heap baseline (reliable per-run measurement)

        // Measured runtime INCLUDES preprocessing (Tier 0 + EUCS) for a fair comparison with the baseline
        ...
        // Root: empty prefix, one instance per sequence with position -1
        try {
            ...
            cpuEndNanos = THREAD_MX.isCurrentThreadCpuTimeSupported()
                    ? THREAD_MX.getCurrentThreadCpuTime() : -1L;
            maxMemory = MemMeter.peakBytes(); // true heap peak for this run
        }
    ...
    private void resetState() {
        stopMemorySampler(); // clean up any sampler thread left by a previously interrupted run
        finalPatterns.clear();
    ...
    /** Single scan: TWU + per-item period. Regularity filtering applied only when useRegPruning is set. */
    ...
    /**
     * Open-addressing primitive hash map for item pairs: memory ~ number of actually co-occurring
     * pairs, instead of a dense numValid^2 matrix (which easily runs out of memory on high-item
     * datasets). Per-sequence deduplication via the stored lastSid field.
     */
    ...
    private void buildEUCS() {
        ...
        int[] seenBefore = new int[numValid];           // dense ids seen in prior events
        ...
        for (int s = 0; s < numSeq; s++) {
            ...
            for (int e = seqEvtStart[s]; e < seqEvtStart[s + 1]; e++) {
                ...
                // I-pairs: same event (unordered, key normalized a<b)
                for (int x = 0; x < m; x++)
                    for (int y = x + 1; y < m; y++) {
                ...
                // S-pairs: item u in an earlier event -> item v in the current event (ordered)
                for (int y = 0; y < m; y++) {
                    int v = evtBuf[y];
                    for (int k = 0; k < nb; k++) mapS.add(pack(seenBefore[k], v), s, su);
                }
                // Update seenBefore AFTER processing S-pairs (same-event items are excluded)
                for (int y = 0; y < m; y++) {
                    int v = evtBuf[y];
                    if (seenStamp[v] != s) { seenStamp[v] = s; seenBefore[nb++] = v; }
                }
            }
            if ((s & 255) == 0) checkMemory(); // memory sample + interrupt check during build
        }

        // Convert to adjacency lists: I is symmetric (both directions), S is directed
        ...
        checkMemory(); // record peak after EUCS construction (temp maps collected by GC)
    }
    ...
    /** Descending quicksort on a pair of parallel arrays (by utility). */
    ...
    /** Projected database: parallel primitive arrays, doubled when full. */
    ...
    /** Extend the current pattern with candidate, compute utility/period, recurse. */
    private void processExtension(InstanceList parent, int candidate, boolean isIExt) {
        ...
        while (i < parent.size) {
            ...
            if (useRegPruning) {
                ...
                if (gap > maxRegularity) { prunedRegularity++; return; } // early break (Tier 3)
            }
            int start = i;
                ...
                for (int q = from; q < to; q++) {
                    if (items[q] == candidate) {
                        ...
                        if (nu > evtMaxUtil[e]) {
                            ...
                            // First match is the earliest position => largest remUtils => valid upper bound
                            if (first) { ruFirst = remUtils[q]; first = false; }
                        }
                        if (isIExt) break;
                        q = evtPosStart[evBase + e + 1] - 1; // unique item in event: jump to the next event
                    }
                }
            }
            if (found) {
        ...
        if (lastSid == -1) return; // candidate never occurs after the prefix
    ...
    /** Generate and filter next-level candidates, then recurse. lastItem = -1 at the root. */
    private void expand(InstanceList projected, int lastItem) {
        ...
        if (boundMode == BOUND_LA_PEU) {
            if (useEUCS && lastItem >= 0) {
                // Promising item set for look-ahead = EUCS neighbours of lastItem meeting the threshold
                ...
                laStamp = ROOT_STAMP; // Tier 0 potential item set
            }
        }

        if (boundMode == BOUND_LA_PEU) computeBoundsLA(projected, laStamp);
        else computeBoundsPEU(projected); // PEU and NONE both require a scan to enumerate candidates

        int nI = collectCandidates(lastItem, true);
        int nS = collectCandidates(lastItem, false);
        resetGlobalAccumulators();

        // Copy before recursion: candBuf is shared across levels
    ...
    /**
     * Standard PEU: bound(x) = maxPrefixUtil(sequence) + u(x at position) + remUtils(position).
     * The S-region needs to be scanned ONCE from after the earliest instance's event (the earliest
     * instance covers every position that later instances can reach), avoiding redundant O(k*suffix) scans.
     */
    private void computeBoundsPEU(InstanceList plist) {
        ...
        while (i < plist.size) {
            ...
            // I-region: the remainder of the event containing each instance
            for (int k = start; k < end; k++) {
                int p = plist.pos[k];
                if (p < 0) continue;
            ...
            // S-region: from the event after the earliest instance to the end of the sequence (instances are position-sorted)
    ...
    private void computeBoundsLA(InstanceList plist, int laStamp) {
        ...
        while (i < plist.size) {
            ...
            for (int k = start; k < end; k++) {
                ...
                long lar = 0;
                // S-region boundary = start position of the event immediately after the event containing p
                ...
                if (p >= 0) {
                    for (int q = sFrom - 1; q > p; q--) { // remainder of the event containing p
                        int it = items[q];
                        long b = pref + utils[q] + lar;
                        if (seqVisitI[it] != s) { seqVisitI[it] = s; seqActI[seqNumI++] = it; seqBoundI[it] = b; }
                        else if (b > seqBoundI[it]) seqBoundI[it] = b;
    ...
    /** Merge per-sequence bounds into the global accumulators; reset per-sequence stamps to -1. */
    ...
    /** Filter candidates through Tier 1 (EUCS) then Tier 2 (upper bound). Returns the candidate count in candBuf. */
        ...
        if (useEUCS && lastItem >= 0) {
            ...
            for (int k = adjStart[d]; k < adjStart[d + 1]; k++) {
                ...
                if (adjUtil[k] < minUtility) {
                    // adjacency list sorted descending: remaining entries are all EUCS-pruned
    ...
    private void popPattern(boolean iExt) {
        patLen--;
        if (!iExt && patLen > 0) patLen--; // remove the event separator
    }
    ...
    private void loadData(String dataPath, String utilPath, double minRatio, double maxRegRatio) throws IOException {
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(dataPath))) {
            ...
            while ((line = br.readLine()) != null) {
                ...
                for (String eventStr : eventStrs) {
                    ...
                    if (evLen > 0) {
                        // Sort items in event ascending for canonical I-extension pattern generation
        ...
        for (int s = 0; s < numSeq; s++) {
            ...
            // remUtils: remaining utility IMMEDIATELY AFTER each position
            long acc = 0;
    ...
    /** Interrupt check only (memory measurement delegated to MemMeter.peakBytes()). */
    ...
    public long getMinUtility() { return minUtility; }       // absolute threshold used
    public int getMaxRegularity() { return maxRegularity; }  // absolute regularity threshold used
    /** CPU time (ms) of the mining thread; robust to machine-sleep noise. -1 if unsupported. */
    ...
    public void printStatistics() {
        ...
        System.out.println("Peak memory                 : " + String.format("%.2f MB", getPeakMemoryMB()));
        System.out.println("Candidates generated        : " + candidateCount);
        System.out.println("Nodes explored              : " + exploredNodes);
        System.out.println("Pruned Tier 1 (EUCS)        : " + prunedEucs);
        System.out.println("Pruned Tier 2 (bound)       : " + prunedBound);
        System.out.println("Pruned node condition (u+ru): " + prunedNodeCond);
        System.out.println("Pruned Tier 3 (regularity)  : " + prunedRegularity);
        System.out.println("===========================================================");
    }
}
